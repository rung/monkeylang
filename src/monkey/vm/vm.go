...
// NewWithGlobalStore is used in repl.
//  this can hold old GlobalStore.
...
func (vm *VM) Run() error {
	...
	for vm.currentFrame().ip < len(vm.currentFrame().Instructions())-1 {
		...
		case code.OpJump:
			...
			vm.currentFrame().ip = pos - 1 // for文でincrementされる
		case code.OpJumpNotTruthy:
			pos := int(code.ReadUint16(ins[ip+1:]))
			// OpJumpNotTruthyはOperandが2byteなので2byte進める
			//  (あとのconditionがFalseのときもうまくipを進められるようにする)
			vm.currentFrame().ip += 2
		...
		case code.OpIndex:
			// stack
			//   index(インデックス指定object)
			//   left(array or hashobject)
			index := vm.pop()
			left := vm.pop()
...
func (vm *VM) executeComparison(op code.Opcode) error {
	...
	switch op {
	// 比較対象がobject.Booleanのとき、Objectをそのまま比較する
	//  object.BooleanのTrue, Falseは常に同じアドレスなのでアドレス比較できる
	case code.OpEqual:
...
func (vm *VM) callClosure(cl *object.Closure, numArgs int) error {
	...
	vm.pushFrame(frame)
	// stack pointerをbp+ローカル変数にする。
	// ローカル変数はbpとのオフセットで表現する
...
func (vm *VM) callBuiltin(builtin *object.Builtin, numArgs int) error {
	...
	// reset stack pointer (Builtin Function doesn't reset it)
...
// isTruthyは条件がtrueかどうか判定する
//  stackにBoolean以外が置かれているとき(数値等)はtrue判定される
